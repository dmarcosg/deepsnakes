...
def active_contour_step(step_n, Fu, Fv, du, dv,
    ...
    for step in range(step_n):
        ...
        du = -max_px_move*np.tanh( (fu - dEb_du.reshape([L,1]) + 2*np.matmul(A/delta_s+B/np.square(delta_s),snake_u))*gamma/max_px_move )*0.1 + du*0.9
        dv = -max_px_move*np.tanh( (fv - dEb_dv.reshape([L,1]) + 2*np.matmul(A/delta_s+B/np.square(delta_s),snake_v))*gamma/max_px_move )*0.1 + dv*0.9
